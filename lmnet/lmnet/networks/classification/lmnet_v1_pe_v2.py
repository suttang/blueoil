...
class LmnetV1(Base):
    ...
    def _get_lmnet_block(self, is_training, is_kernel_quantized, channels_data_format):
        return functools.partial(lmnet_block,
                                 activation=self.activation,
                                 custom_getter=self.custom_getter if is_kernel_quantized else None,
    ...
    def base(self, images, is_training, *args, **kwargs):
        ...
        _lmnet_block = self._get_lmnet_block(is_training, True, channels_data_format)
        _lmnet_half_block = self._get_lmnet_block(is_training, False, channels_data_format)

        self.images = images

        splits = tf.split(images, 3, axis=-1)
        x1 = _lmnet_half_block('emconv1', splits[0], 10, 1)
        x2 = _lmnet_half_block('emconv2', splits[1], 10, 1)
        x3 = _lmnet_half_block('emconv3', splits[2], 10, 1)
        x = tf.concat([x1, x2, x3], axis=-1)

        x = _lmnet_block('conv1', x, 32, 3)
