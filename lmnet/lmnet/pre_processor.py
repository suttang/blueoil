...
class Scale(Processor):
    """Change image scale.
    """
    def __init__(self, scale, with_keys=None, method=PIL.Image.BICUBIC):
        self.scale = scale
    ...
    def __call__(self, **kwargs):
        if self.with_keys is not None:
            processed = {v: scale(kwargs[v], self.scale, method=self.method) for v in self.with_keys if v in kwargs}
        else:
            processed = {}
        
        return dict({**kwargs, **processed})
