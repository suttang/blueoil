...
class Bsds500(Base):
    classes = []
    num_classes = 0
    extend_dir = "BSDS500/data/images"
    ...
    def files(self):
        if self.subset == "train":
            ...
            images_dir = os.path.join(self.data_dir, "val")

        return [filepath for filepath in glob(os.path.join(images_dir, "*.jpg"))]
...
class Bsds500SuperResolution(SuperResolutionBase):
    classes = Bsds500.classes
    num_classes = Bsds500.num_classes
    extend_dir = Bsds500.extend_dir
    available_subsets = Bsds500.available_subsets

    def __init__(self, scale, patch_size, **kwargs):
        self.dataset = Bsds500(**kwargs)
